...
> Статус: **реалізовано** на гілці `feature/auth`. Розділ «Рішення, які пропоную» погоджено без змін.
...
- `AuthServiceTest` — рукописний `RecordingPasswordHasher` записує, **що саме** перевіряли:
  ...
  асертимо наявність самого виклику, бо саме він і створює однаковий час.
