...
Знайдено рев'ю, лишено на потім — не тому, що дрібниця, а тому, що поза скоупом цієї фічі:

- **Реєстрація перелічує адреси за конструкцією:** `409` прямо каже, що email зайнятий. Закривається
  разом із rate limiting, не окремо. Тому вирівняний тайминг логіну — це не повний захист від перебору.
- **Немає обмеження частоти й розміру тіла:** ані на логін, ані на реєстрацію; email не має верхньої
  межі довжини й осідає ключем у мапі.
- **`404`/`405`/`406` віддають порожні тіла** повз `ErrorResponseDto` — клієнт, який завжди парсить
  контракт, спіткнеться на них.
- **`ErrorResponseDto` не має машинного `code`/`field`** — клієнт не прив'яже помилку до поля вводу
  й не локалізує її. Додати пізніше = зміна контракту для всіх клієнтів.
- **`findById` — лінійний скан** під тим самим мьютексом; зникне з індексом за id або з PostgreSQL.
- **Непокриті тестами:** чужі `issuer`/`audience` у верифаєрі, читання `JWT_AUDIENCE` з env,
  токен із `login` (перевіряється лише `isNotBlank`), гілка «валідний підпис, користувача немає».
