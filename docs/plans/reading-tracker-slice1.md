...
- **Недоліки UX, свідомо відкладені:** видалення сесії без підтвердження чи undo.
- **Відоме обмеження: «сьогодні» не переходить через межу 04:00, поки застосунок відкритий.** `today` у зведенні
  перераховується лише на новій емісії даних зі сховища. Якщо застосунок відкритий через межу дня і нових сесій
  немає, зведення показує вчорашній день, доки щось не зміниться. Для Зрізу 1 прийнятно.
  Фікс (пізніше): перераховувати на повернення екрана (resume) і/або таймером до наступної межі дня.
